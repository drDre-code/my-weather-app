...
export const forecast = (latitude: number, longitude: number, callback: (a: string | undefined, b: string | undefined) => void) => {
    ...
    request({ url: url, json: true }, (error, response) => {
        if (error) {
            ...
            const info = `${data.weather[0].description}. It is currently ${temperature} degrees celsius out. The high today is ${tempMax} degrees celsius with a low of ${tempMin} degrees celsius. It feels like ${feelslike} degrees celsius out with humidity of ${humidity}%`;
